...
- **ONLY analyze .dart** - exclude all other file types
- - **Exclude test files** from analysis (files matching patterns: `*_test.dart`, `*.test.js`, `*.test.cjs`, `*.spec.js`, `*.spec.cjs`, `*.test.ts`, `*.spec.ts`, `*_test.py`, `test_*.py`, or located in directories named `test`, `tests`, `__tests__`, or `spec`)
...
- **All test files** (files matching: `*_test.dart`, `*.test.js`, `*.test.cjs`, `*.spec.js`, `*.spec.cjs`, `*.test.ts`, `*.spec.ts`, `*_test.py`, `test_*.py`, or in `test/`, `tests/`, `__tests__/`, `spec/` directories)
...
- **File Type Restriction**: ONLY analyze .dart and .cjs files - ignore all other file types
- **Primary Focus**: All .dart and .cjs files changed in the current push (excluding test files and workflow files)
- **Secondary Analysis**: Check for duplication with existing .dart and .cjs codebase (excluding test files and workflow files)
- **Cross-Reference**: Look for patterns across .dart and .cjs files in the repository
